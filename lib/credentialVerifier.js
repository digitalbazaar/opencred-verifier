...
api.verifyCredential = function(credential) {
  return _getCredentialParams(credential).then(function(results) {
    ...
    tests.verified = true;
    tests.verified = tests.signed && _.chain(_.values(tests)).every().value();
    ...
    .then(function(publicKey) {
      ...
      return _getJson(publicKey.owner)
        ...
        .catch(function(err) {
          ...
          // don't throw to allow next check
        });
    })
    .then(function() {
      // normalize
      ...
      return jsonld.promises().normalize(params.data, options)
        ...
        .catch(function(err) {
          errors.normalize = err;
          // FIXME: remove
          // deprecated old name alias
          errors.normalization = errors.normalize;
          throw results;
        });
    })
